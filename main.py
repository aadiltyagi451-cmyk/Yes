...
def api_get_task():
    if API_TASK_QUEUE:
        return jsonify({"task": API_TASK_QUEUE.pop(0)})
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "AUTO_DONE":
        ...
        if not task_ts:
            await query.message.reply_text("❌ No active task", reply_markup=main_menu_markup(user.id))
            return

        if now - task_ts > 22 * 3600:
            AUTOMATION_USER_TASK_TIME.pop(int(user.id), None)
            AUTOMATION_USER_ACTIVE.pop(int(user.id), None)
            await query.message.reply_text("⏰ Task expired", reply_markup=main_menu_markup(user.id))
            return
