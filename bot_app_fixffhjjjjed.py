...
def init_db():
    ...
    """)
    # Shared worker tables (used by userbot.py but stored in the same DB file)
    cur.execute("""
...
# =========================
USERBOT_DB_PATH = DB  # shared SQLite file for main bot + userbot worker
...
def _queue_userbot_job(job_type: str, user_id: int, payload: dict | None = None) -> int:
    ...
    con = sqlite3.connect(USERBOT_DB_PATH, timeout=10, check_same_thread=False)
    con.row_factory = sqlite3.Row
    try:
        cur = con.cursor()
        cur.execute(
