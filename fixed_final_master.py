...
VIDEO_FILE_ID_CREATE = "BAACAgUAAxkBAAMuacjkYFanrxTNEmA1TOnInEgCbQ8AAv8gAAKQb0lWT29DFeTFiHE6BA"
...
VIDEO_LOGOUT_PATHS = []
VIDEO_FILE_ID_CACHE = {
    "create": VIDEO_FILE_ID_CREATE,
    "logout": VIDEO_FILE_ID_LOGOUT,
    "recovery": VIDEO_FILE_ID_RECOVERY
}
...
AUTOMATION_DONE_QUEUE = []      # FIFO items: {"user": int, "task_id": str}
AUTOMATION_USER_TASK_TIME = {}  # user_id -> latest task assigned ts
AUTOMATION_USER_ACTIVE = {}     # task_id -> task text
AUTOMATION_USER_ACTIVE_TASKS = {}  # user_id -> [task_id, ...]
AUTOMATION_TASK_META = {}       # task_id -> {"user_id": int, "created_at": int, "text": str}
AUTOMATION_MAX_ACTIVE_TASKS = 5


# Persistent automation task store (restart-safe)
def automation_task_upsert(task_id: str, user_id: int, task_text: str, status: str = "active", hold_credit_id: int | None = None):
    def _op():
        con = db()
        cur = con.cursor()
        cur.execute(
            """
            INSERT INTO automation_tasks(task_id, user_id, task_text, status, created_at, updated_at, hold_credit_id, reward_amount)
            VALUES(?,?,?,?,?,?,?,?)
            ON CONFLICT(task_id) DO UPDATE SET
                ...
                hold_credit_id=COALESCE(excluded.hold_credit_id, automation_tasks.hold_credit_id),
                reward_amount=excluded.reward_amount
            """,
            (str(task_id), int(user_id), str(task_text or ""), str(status or "active"), int(time.time()), int(time.time()), hold_credit_id, float(PRE_CREDIT_AMOUNT)),
        )
        con.commit()
        con.close()
    _db_write_retry(_op)
...
def automation_task_set_status(task_id: str, status: str, hold_credit_id: int | None = None):
    def _op():
        con = db()
        cur = con.cursor()
        if hold_credit_id is None:
            cur.execute(
                "UPDATE automation_tasks SET status=?, updated_at=? WHERE task_id=?",
                (str(status), int(time.time()), str(task_id)),
            )
        else:
            cur.execute(
                "UPDATE automation_tasks SET status=?, updated_at=?, hold_credit_id=? WHERE task_id=?",
                (str(status), int(time.time()), int(hold_credit_id), str(task_id)),
            )
        con.commit()
        con.close()
    _db_write_retry(_op)
...
def automation_task_get(task_id: str):
    con = db()
    cur = con.cursor()
    cur.execute("SELECT * FROM automation_tasks WHERE task_id=?", (str(task_id),))
    row = cur.fetchone()
    con.close()
    return row


def automation_load_active_from_db():
    con = db()
    cur = con.cursor()
    cur.execute("SELECT task_id, user_id, task_text, created_at FROM automation_tasks WHERE status IN ('active','failed') ORDER BY created_at ASC")
    rows = cur.fetchall()
    con.close()

    AUTOMATION_USER_ACTIVE.clear()
    AUTOMATION_TASK_META.clear()
    AUTOMATION_USER_ACTIVE_TASKS.clear()

    for r in rows:
        task_id = str(r["task_id"])
        user_id = int(r["user_id"])
        task_text = str(r["task_text"] or "")
        created_at = int(r["created_at"] or int(time.time()))
        AUTOMATION_USER_ACTIVE[task_id] = task_text
        AUTOMATION_TASK_META[task_id] = {"user_id": user_id, "created_at": created_at, "text": task_text}
        AUTOMATION_USER_ACTIVE_TASKS.setdefault(user_id, []).append(task_id)

# API bridge state (bot <-> userbot via Railway internal HTTP)
...
def automation_make_task_id(user_id: int) -> str:
    return f"T{int(user_id)}_{int(time.time() * 1000)}_{random.randint(1000,9999)}"

def automation_task_menu(task_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✔ Done", callback_data=f"AUTO_DONE|{task_id}")],
...
def automation_logout_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
...
def smart_format_task(text: str) -> str:
    ...
    for line in lines:
        ...
        if line.lower().startswith("first name:"):
            ...
            formatted.append(f"First name:`{val}`")

        elif line.lower().startswith("last name:"):
            val = line.split(":",1)[1].strip()
            formatted.append(f"Last name:`{val}`")

        elif line.lower().startswith("email:"):
            val = line.split(":",1)[1].strip()
            formatted.append(f"Email:`{val}`")
    ...
    return "\n\n".join(formatted)
async def automation_send_signal(context: ContextTypes.DEFAULT_TYPE, kind: str, user_id: int, task_id: str | None = None):
    # Queue a task for the userbot to poll over Railway internal HTTP.
    try:
        job = {"type": str(kind or "").lower(), "user": int(user_id)}
        if task_id:
            job["task_id"] = str(task_id)
        API_TASK_QUEUE.append(job)
...
async def automation_assign_task(context: ContextTypes.DEFAULT_TYPE, user_id: int, task_text: str):
    user_id = int(user_id)
    active = AUTOMATION_USER_ACTIVE_TASKS.get(user_id, [])
    if len(active) >= AUTOMATION_MAX_ACTIVE_TASKS:
        await context.bot.send_message(
            chat_id=user_id,
            text="❌ You already have 5 active tasks. Complete one first.",
            reply_markup=main_menu_markup(user_id)
        )
        return

    task_id = automation_make_task_id(user_id)
    task_text = smart_format_task(task_text)

    AUTOMATION_USER_TASK_TIME[user_id] = int(time.time())
    AUTOMATION_USER_ACTIVE[task_id] = task_text
    AUTOMATION_TASK_META[task_id] = {
        "user_id": user_id,
        "created_at": int(time.time()),
        "text": task_text,
    }
    active.append(task_id)
    AUTOMATION_USER_ACTIVE_TASKS[user_id] = active

    send_text = f"🆔 Task ID: `{task_id}`\n\n{task_text}"

    await context.bot.send_message(
        chat_id=user_id,
        text=send_text,
        parse_mode="Markdown",
        reply_markup=automation_task_menu(task_id)
    )

async def automation_send_result(
    ...
    if not task_id:
        for item in AUTOMATION_DONE_QUEUE:
            if int(item.get("user", 0)) == uid:
    ...
    if ok:
        ...
        if task_id:
            AUTOMATION_USER_ACTIVE.pop(task_id, None)
            AUTOMATION_TASK_META.pop(task_id, None)
            tasks = AUTOMATION_USER_ACTIVE_TASKS.get(uid, [])
            if task_id in tasks:
                tasks.remove(task_id)
            AUTOMATION_USER_ACTIVE_TASKS[uid] = tasks

        await context.bot.send_message(
            chat_id=uid,
            text=msg,
            reply_markup=automation_logout_menu()
        )
    else:
        if not msg:
            msg = "❌ Task Not Completed"

        retry_markup = automation_task_menu(task_id) if task_id else None
...
async def handle_automation_admin_message(update: Update, context: ContextTypes.DEFAULT_TYPE, txt: str) -> bool:
    ...
    if raw.startswith("✅") or raw.startswith("❌") or "task completed" in lowered or "task not completed" in lowered:
        if AUTOMATION_DONE_QUEUE:
            item = AUTOMATION_DONE_QUEUE.pop(0)
            target_uid = int(item.get("user", 0))
            task_id = item.get("task_id")
            ok = raw.startswith("✅") or "task completed" in lowered
            await automation_send_result(context, target_uid, ok, automation_convert_price(raw), task_id=task_id)
            return True
...
# =========================
# INTERNAL API (Railway private networking)
# =========================
def _api_submit_coro(coro):
    global APP_LOOP
    if APP_LOOP is None:
        raise RuntimeError("Bot loop not ready")
    return asyncio.run_coroutine_threadsafe(coro, APP_LOOP).result(timeout=30)

@api_flask.route("/health", methods=["GET"])
def api_health():
    return jsonify({"ok": True, "queue": len(API_TASK_QUEUE)})

@api_flask.route("/get-task", methods=["GET"])
def api_get_task():
    if API_TASK_QUEUE:
        job = API_TASK_QUEUE.pop(0)
        return jsonify(job)
    return jsonify({})

@api_flask.route("/task", methods=["POST"])
def api_task():
    ...
    if not user_id or not task:
        return jsonify({"ok": False, "error": "missing user_id/task"}), 400
    try:
        _api_submit_coro(automation_assign_task(APP_INSTANCE, user_id, task, task_id=task_id))
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "error": repr(e)}), 500

@api_flask.route("/result", methods=["POST"])
def api_result():
    data = request.get_json(silent=True) or {}
    user_id = int(data.get("user_id") or 0)
    ...
    task_id = str(data.get("task_id") or "").strip() or None
    if not user_id or status not in {"success", "fail"}:
        return jsonify({"ok": False, "error": "invalid payload"}), 400
    try:
        if task_id:
            globals()["AUTOMATION_DONE_QUEUE"] = [x for x in AUTOMATION_DONE_QUEUE if not (int(x.get("user", 0)) == int(user_id) and str(x.get("task_id") or "") == task_id)]
        _api_submit_coro(automation_send_result(APP_INSTANCE, user_id, status == "success", message_text, task_id=task_id))
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "error": repr(e)}), 500

def run_internal_api():
    api_flask.run(host="0.0.0.0", port=API_PORT, debug=False, use_reloader=False)
...
async def send_recovery_video(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    try:
        await context.bot.send_document(
            chat_id=chat_id,
            document=RECOVERY_FILE_ID,
            caption="✅ How to add Recovery email"
        )
    except Exception:
        await context.bot.send_message(chat_id=chat_id, text="❌ Recovery file send failed.")
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt_is(txt, "menu_register"):
        ...
        if AUTOMATION_MODE:
            active = AUTOMATION_USER_ACTIVE_TASKS.get(int(user.id), [])
            if len(active) >= AUTOMATION_MAX_ACTIVE_TASKS:
                await update.message.reply_text("❌ You already have 5 active tasks. Complete one first.", reply_markup=main_menu_markup(user.id))
                return
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # AUTOMATION BUTTONS
    # =========================
    if data.startswith("AUTO_DONE|"):
        task_id = data.split("|", 1)[1].strip()
        meta = AUTOMATION_TASK_META.get(task_id)
        if not meta or int(meta.get("user_id", 0)) != int(user.id):
            await q.message.reply_text("❌ Task not found", reply_markup=main_menu_markup(user.id))
            return

        now = int(time.time())
        task_ts = int(meta.get("created_at", 0) or 0)

        if not task_ts:
            await q.message.reply_text("❌ No active task", reply_markup=main_menu_markup(user.id))
            return

        if now - task_ts > 22 * 3600:
            AUTOMATION_TASK_META.pop(task_id, None)
            AUTOMATION_USER_ACTIVE.pop(task_id, None)
            tasks = AUTOMATION_USER_ACTIVE_TASKS.get(int(user.id), [])
            if task_id in tasks:
                tasks.remove(task_id)
            AUTOMATION_USER_ACTIVE_TASKS[int(user.id)] = tasks
            await q.message.reply_text("⏰ Task expired", reply_markup=main_menu_markup(user.id))
            return

        if not any(int(x.get("user", 0)) == int(user.id) and str(x.get("task_id") or "") == task_id for x in AUTOMATION_DONE_QUEUE):
            AUTOMATION_DONE_QUEUE.append({"user": int(user.id), "task_id": task_id})
        ...
        try:
            await automation_send_signal(context, "DONE", int(user.id), task_id=task_id)
        except Exception:
            pass

        await q.message.reply_text("⏳ Checking...")
        return

    if data == "VID_CREATE":
        if VIDEO_FILE_ID_CREATE:
            await q.message.reply_document(document=VIDEO_FILE_ID_CREATE)
        else:
            await q.message.reply_text("❌ Create video not set")
        return

    if data == "VID_LOGOUT":
        if VIDEO_FILE_ID_LOGOUT:
            await q.message.reply_document(document=VIDEO_FILE_ID_LOGOUT)
        else:
            await q.message.reply_text("❌ Logout video not set")
        return

    if data == "VID_RECOVERY":
        if VIDEO_FILE_ID_RECOVERY:
            await q.message.reply_document(document=VIDEO_FILE_ID_RECOVERY)
        else:
            await q.message.reply_text("❌ Recovery video not set")
        return
...
async def _post_init(application: Application):
    ...
    try:
        automation_load_active_from_db()
    except Exception as e:
        print(f"[AUTOMATION] restore failed: {e!r}")
